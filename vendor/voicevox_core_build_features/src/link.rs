...
pub fn link(attempt_pkg_config: bool) -> anyhow::Result<()> {
    if *HOST == *TARGET && attempt_pkg_config {
        ...
        match pkg_config::Config::new()
            ...
            .probe(&format!("{DLL_PREFIX}voicevox_onnxruntime"))
        {
            Ok(_) => return Ok(()),
            Err(err) => {
    ...
    if rust_target.contains(&"windows") {
        println!("cargo::rustc-link-lib=static=voicevox_onnxruntime"); // import libraryの方
    } else {
        println!("cargo::rustc-link-lib=dylib=voicevox_onnxruntime");
    }
