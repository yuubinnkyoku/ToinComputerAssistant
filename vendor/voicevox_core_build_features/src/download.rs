...
const ORT_RELEASE_TAG_PREFIX: &str = "voicevox_onnxruntime";
const ORT_ASSET_PREFIX: &str = "voicevox_";
...
pub fn download(add_link_search: bool) -> anyhow::Result<()> {
    ...
    {
        let release_tag = format!("{ORT_RELEASE_TAG_PREFIX}-{VERSION}");
        let asset_name = format!("{ORT_ASSET_PREFIX}{artifact_name}-{VERSION}.tgz");
        ...
        if let Some(lib_sha256) = lib_sha256 {
            verify(lib_content, &lib_sha256)?;
        } else {
            println!(
                "cargo::warning=no lib-sha256 configured for target '{}'; skipping checksum verification for '{}'",
                *TARGET, lib_versioned_file_name
            );
        }
        ...
        if let Some(importlib_path) = importlib_path {
            ...
            let importlib_file_name =
                importlib_file_name().expect("windows target should have import library");
            let importlib_content = &extract_lib(tgz, importlib_file_name)?;
...
fn lib_versioned_file_name(version: &str) -> anyhow::Result<String> {
    ...
    if rust_target.contains(&"windows") {
        Ok("voicevox_onnxruntime.dll".to_owned())
    } else if rust_target.contains(&"apple") {
        Ok(format!("libvoicevox_onnxruntime.{version}.dylib"))
    } else if rust_target.contains(&"unknown") && rust_target.contains(&"linux") {
        Ok(format!("libvoicevox_onnxruntime.so.{version}"))
    } else if rust_target.contains(&"linux") && rust_target.contains(&"android") {
        Ok("libvoicevox_onnxruntime.so".to_owned())
    } else {
...
fn lib_unversioned_file_name() -> anyhow::Result<&'static str> {
    ...
    if rust_target.contains(&"windows") {
        Ok("voicevox_onnxruntime.dll")
    } else if rust_target.contains(&"apple") {
        Ok("libvoicevox_onnxruntime.dylib")
    } else if rust_target.contains(&"linux") {
        Ok("libvoicevox_onnxruntime.so")
    } else {
        bail!("unknown target tuple: {}", *TARGET);
    }
}
...
fn importlib_file_name() -> Option<&'static str> {
    let rust_target = TARGET.split('-').collect::<Vec<_>>();
    (rust_target.contains(&"windows")).then_some("voicevox_onnxruntime.lib")
}
